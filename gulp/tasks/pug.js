...
import { gulp, plugins, args, config, basePath, dirs, taskTarget, browserSync, pjson, notification_icon_location } from '../config/shared-vars';
...
import { is_string, is_int, is_numeric, is_array, in_array, isset, end } from '../helpers/php-to-js-translators';
import { convertLink, getNavMap, generateLink } from '../helpers/navMap-helpers';
...
import { navMap_pre_defaults, navMap_post_defaults, defaultTemplate } from '../config/navMap-user-defaults';
		...
		function generate_page(map, index, parent, hasExclusive, templateList){
			...
			function run_page_generation(){
				...
				fs.stat(map.pageLocation+'_content.pug', function(err, stat) {
					...
					console.log(`
 - ${plugins.util.colors.yellow(map.title)} | ${plugins.util.colors.green(map.template)} | [${map.location}]
   ${map.path}`);
			...
			var exclusivityTest = hasExclusive && map.generate === 'exclusive' || map.location.length === 1 && map.location[0] === 0;
			var page_is_allowed = map.inTemplateList && !map.templateListItem || map.templateListItem && templates_not_in_nav.indexOf(map.template) > -1 || args.production || args.all;
		...
		dir.files(templatesFolder, function(err, files) {
			...
			templateFiles.forEach((file, i) => {
				templateFiles[i] = file.substr(templatesFolder.length + 1).replace('.pug','');
			});
			...
			siteData.navMap.subnav.forEach((map, index) => {
				read_navMap(map, index, (pageMap, i, parentMap)=>{
					...
					if(pageMap.generate === 'exclusive' && !args.production && !args.all){
						hasExclusive = true;
					}

					//collects all the templates
					var
						not_in_template_list_already = templateHits.indexOf(pageMap.template) < 0,
						not_under_home_page_as_default_template = !(pageMap.location[0] === 0 && pageMap.template === defaultTemplate),
						not_a_link_to_another_page = pageMap.link !== 'undefined' ? !is_array(pageMap.link) /*&& pageMap.link.substr(0, 4) !== 'http'*/ : true,
						not_an_external_link = typeof pageMap.link !== 'undefined' ? !/^http/.test(pageMap.link) : true,
						template_is_allowed = not_in_template_list_already && not_under_home_page_as_default_template && not_a_link_to_another_page && not_an_external_link;
					...
					if (template_is_allowed){
						...
						if (isset(templateMap)){
							...
							console.log(`${plugins.util.colors.bold.red('WARNING!')} The "${pageMap.template}" template is defined in the nav map but the file was not found.`)
						}
			...
			waitForFile(path.join(navMapDest, 'content.json'), ()=> {
				...
				outputJSON
					...
					.pipe(gulp.dest(navMapDest));

				//waits for index.pug to exist before trying to compile
				waitForFile(path.join(dirs.source, dirs.pages, homeFolder, 'index.pug'), { notify: false }, ()=> {

					//Tells you what pages have been generated and where to find them
					console.log(`
   ${plugins.util.colors.bold('The full generated navMap for debugging can be found here:')}
...
`
   - Use the ${plugins.util.colors.yellow('--production')} flag when running tasks
     ${plugins.util.colors.grey('(This is to prevent accidental partial compiles during code releases)')}`
						];

						if (hasExclusive){
							options.push(`
   - Remove all ${plugins.util.colors.yellow('"generate":"exclusive"')} commands from navMap.json
  ...
  ${plugins.util.colors.yellow.bold('Not compiling the full site?')}
   ...
   ${plugins.util.colors.yellow('"generate":"exclusive"')} to the pages that you are currently
   working on in the navMap then use the ${plugins.util.colors.yellow('--all')} flag when
...
`);
		...
		function compile(){
			...
			.pipe(plugins.plumber((error)=>{
				if(a === 0) {
					...
					a++;
					return notifier.notify({title: 'Pug Error', message: error.message, icon: notification_icon_location+'gulp-error.png'});
				}
			}))
			.pipe(plugins.pug({
				pug: pug,
			...
			.pipe(gulp.dest(dest))
			.on('end', ()=>{
				notifier.notify({title:"Pug", message: 'HTML compiled successfully', icon: notification_icon_location+'gulp.png'})
	...
	gulp.task('generate-redirect', ()=>{
		if (config.basePath && config.basePath !== '' && config.basePath !== '/' && config.redirect === true){
	...
	gulp.task('pug', ['symbolize-svgs','clean:pages:indexes', 'generate-redirect'], ()=> {
