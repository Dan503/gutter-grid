...
import { gulp, plugins, args, config, dirs, taskTarget, browserSync, notification_icon_location, jsWatch } from '../config/shared-vars';
	...
	let browserifyTask = (files) => {
		return files.map((entry) => {
			...
			let customOpts = {
				...
				paths: ['./node_modules', './src/'+dirs.modules, './src/'+dirs.scripts, './'+dirs.jsUtils]
			};
			...
			let rebundle = function() {
				...
				bundler.bundle()
					.on('error', function(err) {
						hasError = true;
						notifier.notify({title: 'Scripts error', message: err.message, icon: notification_icon_location+'gulp-error.png'});
						console.log(`\n ${plugins.util.colors.red.bold('JS failed to compile:')} ${plugins.util.colors.yellow(err)}\n`);
						this.emit('end');
					})
					.pipe(vsource(entry))
					.pipe(buffer())
					...
					.pipe(plugins.rename(function(filepath) {
						...
						filepath.dirname = path.join(dirs.assets, dirs.scripts.replace(/^_[0-9]_/,''));
					}))
					...
					.on('end', function() {
						if (!hasError){
							...
							notifier.notify({title: 'Scripts', message: path.basename(entry)+" compiled successfully", icon: notification_icon_location+'gulp.png'});
							...
							const filePath = entry.replace(dirs.source,'').replace(file, '').replace(/^[\\]|[\\]$/g,'');

							if (args.export && file === entries.js) exportAsset({ file, path:filePath });
	...
	gulp.task('scripts', (done) => {
		...
		dir.files(path.join('./', dirs.source, dirs.modules), function(err, files) {
			...
			let jsFiles = files.filter(function (file) {
				return file.indexOf('.js') > -1 && path.parse(file).name.substr(0,1) !== '_';
			});

			jsFiles.forEach((file, i) => {
				let modName = path.parse(file).name;
				moduleFiles[modName] = file.replace(/\\/g, '/').replace(dirs.source, '..').replace('.js','');
			});
