...
import { gulp, plugins, args, config, dirs, taskTarget, browserSync, notification_icon_location, bp } from '../config/shared-vars';
...
export default function() {
	...
	let dest = path.join(taskTarget, config.basePath, dirs.assets, dirs.styles.replace(/^_0_/, ''));
	...
	gulp.task('sass:compile', () => {
		...
		gulp.src([
			...
			.pipe(plugins.plumber((error)=>{
				if(a === 0) {
					...
					a++;
					return notifier.notify({title: 'Sass Error', message: error.message, icon: notification_icon_location+'gulp-error.png'});
				}
			}))
		...
		gulp.src([
			...
			exportAsset({file: 'noMQs-'+entries.css, path: dirs.styles, icons: false, notify: false});
