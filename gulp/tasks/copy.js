...
export default function() {
	...
	gulp.task('copy:scripts', ()=>{
		let dest = path.join(taskTarget, config.basePath, dirs.assets, dirs.scripts.replace(/_[0-9]_/, ''));
		return gulp.src([
	...
	gulp.task('copy:root-files', ()=>{
		return gulp.src(dirs.source+'/_root-files/*')
    ...
    if (content_src() !== false){
	    return gulp.src(asset_src+'/'+dirs.pages+'/**/'+dirs.assets+'/**/*')
	...
	gulp.task('copy', ['copy:scripts','copy:root-files'], () => {
		...
		return gulp.src([
				...
				'!' + path.join(dirs.source, '/'+dirs.pages+'/**/'+dirs.assets+'/*'),
