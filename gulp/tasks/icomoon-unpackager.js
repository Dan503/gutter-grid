...
export default function() {
	...
	gulp.task('copy-icons', () => {
		let iconDest = path.join(taskTarget, config.basePath, dirs.assets, dirs.icons.replace('_',''));
  ...
  gulp.task('icomoon-unpackager', ['copy-icons'], () => {
  	gulp.src(path.join(dirs.source, dirs.icons,'variables.scss'))
        ...
        .pipe(plugins.replace(/\$icon-([A-z0-9\-_]*):\s\"\\([A-z0-9]*)";/g, '\t$1: "$2",'))
		//removes the "$icomoon-font-path: "fonts" !default;" bit
        .pipe(plugins.replace(/\$icomoon-font-path: \"([A-z0-9]*)\" \!default\;/g, ''))
		.pipe(plugins.header('$icons: (\n'))
		.pipe(plugins.footer(');'))
		.pipe(plugins.header('/*This is an automatically generated file. DO NOT EDIT! Update the icon font by dumping the contents of icomoon packages into the icomoon folder*/\n'))
