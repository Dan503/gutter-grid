...
export default function exportAsset({ file, path, notify = true, icons = false }){
	if (args.export){
		file = file.replace('.scss', '.css');
		path = path.replace(/_([0-9]_)?/,'');
		const sourceFile = [taskTarget, config.basePath, dirs.assets, path, file].join('/');
		waitForFile(sourceFile, {notify: false}, ()=>{
			if (notify) console.log(`\n${[path,plugins.util.colors.yellow(file)].join('/')} ${icons ? 'and icons ' : ''}exported to:`);
			config.exportLocations.forEach(location => {
				...
				if (icons){
					...
					gulp.src([dirs.source, dirs.icons, '/**/*'].join('/'))
