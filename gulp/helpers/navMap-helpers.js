...
import { is_string, is_int, is_numeric, is_array, in_array, isset, end } from './php-to-js-translators';
	...
	function generateLink (pageMap, i, parentMap){
		...
		if (isset(pageMap.link)){
			if (typeof pageMap.link === 'string' && pageMap.link.substring(0, 4) !== "http"){
				...
				if (pageMap.link.substring(0, 1) !== "/"){
					if (pageMap.link.indexOf(config.basePath) < 0){
						...
						pageMap.link = config.basePath.length > 0 ? `/${config.basePath}/${pageMap.link}` : '/'+pageMap.link;
					} else {
				...
				var fileExtension = pageMap.link.substr(pageMap.link.lastIndexOf('.')+1);
