...
export default function waitForFile(path, settings = {}, callback = ()=>{}){
	...
	if (settings.notify) console.log(plugins.util.colors.yellow('Waiting for:'),path);
	...
	var interval = setInterval(()=>{
		fs.stat(path, function(err, stat) {
			if (err == null){
				if (settings.notify) console.log(plugins.util.colors.green('Found:'), path);
				...
				console.log(plugins.util.colors.red.bold("Error:"), path + " was not found");
				return false;
			};
		});
