...
if (!Array.prototype.indexOf) {
	Array.prototype.indexOf = function (r) {
		...
		for ((i = i < 0 ? Math.ceil(i) : Math.floor(i)) < 0 && (i += t); i < t; i++)
			if (i in this && this[i] === r) return i;
		return -1;
	};
}
...
$(document).ready(function () {
	...
	function DemoCode(elem) {
		...
		this.add_html_class = function () {
			...
			var replacement = html.replace(
				regex,
				'&lt;div class="grid grid--cols-$1 grid--wrap"&gt;'
			);
		...
		this.add_mixin_wrap_setting = function () {
			...
			if (html.indexOf('$wrap: false') === -1) {
				...
				var newHTML = html.replace(
					/include grid\((.+)\)/gi,
					'include grid($1, $wrap: true)'
				);
				//long hand version
				newHTML = newHTML.replace(
					/include grid\(\$cols: ([0-9]+?),  \$gutter: ([0-9A-z]+?)(.*)\)/gi,
					'include grid($cols: $1, $gutter: $2, $3, $wrap: true)'
				);
	...
	function Demo(demoElem) {
		...
		this.can_modify = function () {
			var excluded_ids = ['11-horizontal-cell-alignments-3'];
